...
func _ready() -> void:
	EventBus.message_popup.connect(
		func(msg: String, ok_func : Callable) -> void: call_deferred("_on_message_popup", msg, ok_func)
	)


func _on_message_popup(msg: String, ok_func : Callable) -> void:
...
#	)
	...
	if ok_func:
		...
		ok_btn.pressed.connect(func() -> void: ok_func.call())
